// src/shared/integrations/bnet/client.ts
...
class BnetClient {
  ...
  private async fetchWithRetry(
    ...
    try {
      ...
      if (res.status === 429 || res.status >= 500) {
        if (attempt === retries) {
          throw new Error(
            `Battle.net API error (${res.status}) on ${finalUrl} after ${retries} attempts.`,
          );
        }

        const waitTime = Math.pow(2, attempt) * 500;
        await new Promise((resolve) => setTimeout(resolve, waitTime));
        return await this.fetchWithRetry(finalUrl, token, retries, attempt + 1);
      }
  ...
  async fetchApi(
    url: string,
    namespace: 'dynamic-eu' | 'static-eu',
    retries = 3,
  ): Promise<any> {
    ...
    return await this.fetchWithRetry(finalUrl, token, retries, 1);
  }
  ...
  async getInstance(instanceId: number) {
    return this.fetchApi(
      `https://eu.api.blizzard.com/data/wow/journal-instance/${instanceId}`,
      'static-eu',
    );
  }
