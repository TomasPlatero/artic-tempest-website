...
export function assertCookieAttribute(
  ...
  const found = parts.find(
    (p) => p.toLowerCase() === attrLower || p.toLowerCase().startsWith(`${attrLower}=`),
  );
  expect(found, `Cookie attribute '${attr}' not found in: ${setCookie}`).toBeTruthy();
  ...
  if (value === 'true' || value === 'false') {
    ...
    const expected = attr.endsWith('=') ? `${attr}${value}` : `${attr}=${value}`;
    expect(
