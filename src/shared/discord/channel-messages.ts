...
const cache = new Map<string, { data: DiscordChannelMessage[]; fetchedAt: number }>()
...
let guildContextCache: { guildId: string; ctx: GuildMemberContext; fetchedAt: number } | null =
  null
...
function formatDiscordTime(iso: string): string {
  ...
  const diffDays = Math.round((dayStartMs(now) - dayStartMs(date)) / 86_400_000)
...
function mapMessage(raw: any): DiscordChannelMessage {
  ...
  return {
    ...
    author: {
      id: raw.author?.id ?? "",
      displayName: raw.author?.global_name || raw.author?.username || "Desconocido",
...
function resolveDiscordSyntax(content: string, ctx: GuildMemberContext): string {
  return content
...
async function getGuildMemberContext(guildId: string): Promise<GuildMemberContext> {
  if (
    guildContextCache &&
  ...
  try {
    ...
    for (const member of members) {
      ...
      const coloredRole = coloredRoles.find((role) => memberRoleSet.has(role.id))
      colorByUserId.set(
        user.id,
    ...
    const ctx: GuildMemberContext = {
      ...
      colorByRoleId,
      nameByChannelId: new Map(channels.map((channel) => [channel.id, channel.name])),
    }
...
export async function getDiscordChannelMessages(
  ...
  try {
    ...
    if (!res.ok) {
      console.error(`[DISCORD] fetch channel ${channelId} messages failed: ${res.status}`)
      return []
    }
