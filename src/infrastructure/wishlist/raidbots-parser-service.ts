// src/infrastructure/wishlist/raidbots-parser-service.ts
...
export class RaidbotsParserService {
     ...
     */
    static translateBonusIds(bonusIds: number[]): { track: string; ilvl?: number } {
     ...
     */
    static parseReport(data: any): { items: WishlistItem[]; dpsGain: number; percentGain: number } {
        if (!data.sim || !data.sim.profilesets) {
            throw new Error("Invalid Raidbots report format");
        }

        const baselineDps = data.sim.statistics?.raid_dps?.mean || data.sim.dps?.mean || 0;
        ...
        profilesets.forEach((ps: any) => {
            ...
            const slot = itemData.slot || this.extractSlotFromName(ps.name) || "Unknown";
            ...
            if (!bestGainBySlot.has(normalizedSlot) || dpsGain > (bestGainBySlot.get(normalizedSlot) || 0)) {
                ...
                const wishItemEntry: WishlistItem = {
                    ...
                    percentGain: baselineDps > 0 ? Number(((dpsGain / baselineDps) * 100).toFixed(2)) : 0,
                    status: 'needed',
        ...
        if (data.sim?.options?.type === "topgear") {
            const bestResult = profilesets.sort((a: any, b: any) => (b.mean || 0) - (a.mean || 0))[0];
        ...
        return {
            ...
            percentGain: baselineDps > 0 ? Number(((totalDpsGain / baselineDps) * 100).toFixed(2)) : 0
        };
    }
