// src/infrastructure/wishlist/wishlist-service.ts
...
import { fetchCharacterEquipment, fetchItemData } from "@/infrastructure/bnet/bnet-client";
...
export class WishlistService {
     ...
     */
    static async importFromRaidbots(memberId: string, raidbotsUrl: string): Promise<Wishlist> {
        // 1. Fetch Report Data
        ...
        const equipment = await fetchCharacterEquipment(member.realm_slug, member.character_name.toLowerCase());
        ...
        for (const pItem of parsed.items) {
            ...
            if (bnetItem && !ItemFilterService.canSpecUseItem(member.class_id, member.spec_name, bnetItem)) {
        ...
        const { data: wishlist, error: wlError } = await supabaseAdmin.from("wishlists")
            .upsert({
                member_id: memberId,
        ...
        await supabaseAdmin.from("wishlist_items").delete().eq("wishlist_id", wishlist.id);
        ...
        const { error: itemsError } = await supabaseAdmin.from("wishlist_items").insert(itemsToInsert);
    ...
    static async getWishlist(memberId: string): Promise<Wishlist | null> {
        const { data: wishlist } = await supabaseAdmin.from("wishlists")
