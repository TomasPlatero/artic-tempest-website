...
function normalizePovTabs(value: unknown) {
  if (!Array.isArray(value)) return [];

  return value.map((tab: any) => ({
    character_name: String(tab?.character_name || ''),
    class_id: Number(tab?.class_id || 0),
    subtitle: String(tab?.subtitle || ''),
    youtube_url:
      toSafeYouTubeEmbed(String(tab?.youtube_url || '')) ||
      String(tab?.youtube_url || ''),
  }));
}
...
export async function POST(req: Request) {
  try {
    ...
    const normalizedPovTabs = normalizePovTabs(pov_tabs);
...
export async function PATCH(req: Request) {
  try {
    ...
    if ('pov_tabs' in updates) {
      normalizedUpdates.pov_tabs = normalizePovTabs((updates as any).pov_tabs);
    }
