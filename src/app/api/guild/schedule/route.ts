...
function getBackgroundUrl(destination: string) {
  ...
  if (destLower.includes('voidspire') || destLower.includes('aguja'))
    return '/assets/images/raids/voidspire.webp';
  if (
    destLower.includes('dreamrift') ||
    destLower.includes('falla') ||
    destLower.includes('sueño') ||
    destLower.includes('onírica')
  )
    return '/assets/images/raids/dreamrift.webp';
  if (
    destLower.includes("quel'danas") ||
    destLower.includes('sunwell') ||
    destLower.includes('marcha')
  )
    return '/assets/images/raids/marchonqueldanas.webp';
...
export async function POST(request: Request) {
  try {
    ...
    const { data: guildData } = await supabaseAdmin
      .from('guilds_managed')
      .select('guild_id, last_schedule_sync')
      .limit(1)
      .single();
    ...
    if (!force && guildData.last_schedule_sync) {
      const msSinceSync =
        now.getTime() - new Date(guildData.last_schedule_sync).getTime();
      if (msSinceSync < 6 * 60 * 60 * 1000) {
        return NextResponse.json({
          message: 'Skipped sync, too soon',
          synced: false,
        });
      }
    }
    ...
    if (!schedules || schedules.length === 0) {
      return NextResponse.json({
        message: 'No active schedule to sync',
        synced: true,
      });
    }
    ...
    const startFrom =
      startDate && !isNaN(startDate.getTime()) ? startDate : today;
    ...
    if (eventsToInsert.length === 0) {
      await supabaseAdmin
        .from('guilds_managed')
        .update({ last_schedule_sync: new Date().toISOString() })
        .eq('guild_id', guildId);
      return NextResponse.json({
        message: 'No new events needed',
        synced: true,
      });
    }

    const { data: existingEvents, error: existErr } = await supabaseAdmin
      .from('guild_events')
      ...
      .lte(
        'event_date',
        new Date(
          today.getTime() + SYNC_DAYS_AHEAD * 24 * 60 * 60 * 1000,
        ).toISOString(),
      );
    ...
    const finalInsertBatch = eventsToInsert.filter((evt) => {
      const duplicate = existingEvents?.find((ex) => {
        const isSameTime =
          new Date(ex.event_date).getTime() ===
    ...
    await supabaseAdmin
      ...
      .eq('guild_id', guildId);
    return NextResponse.json({
      message: `Synced ${finalInsertBatch.length} events`,
      synced: true,
    });
  } catch (error: any) {
