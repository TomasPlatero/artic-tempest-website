...
export async function PATCH(req: Request) {
	try {
		...
		if (discord_streams_channel_id !== undefined)
			discordUpdates.discord_streams_channel_id =
				discord_streams_channel_id || null;
		if (discord_recruitment_channel_id !== undefined)
			discordUpdates.discord_recruitment_channel_id =
				discord_recruitment_channel_id || null;
		if (discord_requested_scopes !== undefined)
			discordUpdates.discord_requested_scopes =
				discord_requested_scopes || null;
		...
		if (hasDiscordFields) {
			const { error: discordError } = await supabaseAdmin
				.from("app_discord")
				.upsert({ id: 1, ...discordUpdates }, { onConflict: "id" });

			if (discordError) {
				console.error(discordError);
				return new NextResponse("Error de base de datos (discord)", {
					status: 500,
				});
			}
		}
		...
		if (bnet_client_id !== undefined || bnet_client_secret !== undefined) {
				...
				.upsert(
					{
						id: 1,
						updated_at: new Date().toISOString(),
						...(bnet_client_id !== undefined
							? { bnet_client_id: bnet_client_id || null }
							: {}),
			...
			if (bnetError) {
				...
				return new NextResponse("Error de base de datos (battle.net)", {
					status: 500,
				});
			}
		}

		if (wowaudit_api_key !== undefined && wowaudit_api_key !== MASK) {
				...
				.upsert(
					{
						id: 1,
						updated_at: new Date().toISOString(),
						wowaudit_api_key: wowaudit_api_key || null,
					},
					{ onConflict: "id" },
				);
			...
			if (wowauditError) {
				...
				return new NextResponse("Error de base de datos (wowaudit)", {
					status: 500,
				});
			}
		}
		...
		if (wcl_client_id !== undefined || wcl_client_secret !== undefined) {
			const { error: wclError } = await supabaseAdmin.from("app_wcl").upsert(
				{
					id: 1,
					updated_at: new Date().toISOString(),
					...(wcl_client_id !== undefined
						? { wcl_client_id: wcl_client_id || null }
						: {}),
			...
			if (wclError) {
				...
				return new NextResponse("Error de base de datos (wcl)", {
					status: 500,
				});
			}
		}
