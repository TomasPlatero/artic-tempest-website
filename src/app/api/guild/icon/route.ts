...
export async function POST(request: Request) {
  try {
    ...
    const { data: guild, error: guildResError } = await supabaseAdmin
      .from('guilds_managed')
      .select('guild_id')
      .limit(1)
      .single();
    ...
    if (guildResError || !guild) {
      ...
      return NextResponse.json(
        { error: 'No hay ninguna hermandad configurada' },
        { status: 400 },
      );
    }
    ...
    const {
      data: { publicUrl },
    } = supabaseAdmin.storage.from('guild_assets').getPublicUrl(fileName);

    // Update database
    const updateData = isMobile
      ? { mobile_icon_url: publicUrl }
      : { icon_url: publicUrl };
