    ...
    try {
      ...
      `)
        ...
        const buildTree = async (
            ...
            parentAllowedRoles: Set<string> | null = null,
        ) => {
            ...
            const permissionResults = await Promise.all(
                children.map(async (item) => {
                    if (item.app_id) {
                        const perm = await getAppPermission(
                            roleLevel,
                            item.app_id as any,
                        );
            ...
            const permMap = new Map(
                permissionResults.map((r) => [r.itemId, r.canView]),
            );
            ...
            const results = await Promise.all(
                children.map(async (item) => {
                    ...
                    const currentAllowedRoles =
                        itemRoles.length > 0
                            ? new Set<string>(itemRoles)
                            : parentAllowedRoles;

                    // Check if user has one of the allowed roles (if restricted)
                    if (
                        currentAllowedRoles &&
                        !currentAllowedRoles.has(roleLevel)
                    ) {
                        return null;
                    }
                    ...
                    const subItems: any[] = await buildTree(
                        item.id,
                        currentAllowedRoles,
                    );
                    return {
