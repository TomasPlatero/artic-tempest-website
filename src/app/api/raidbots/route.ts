...
export async function GET(req: Request) {
  try {
    ...
    if (!match)
      return new NextResponse('URL de Raidbots no válida', { status: 400 });
    ...
    if (!res.ok) {
      return new NextResponse('No se pudo obtener datos de Raidbots', {
        status: res.status,
      });
    }
    ...
    if (!data.sim || !data.sim.profilesets) {
      return new NextResponse(
        'El reporte no contiene set de perfiles (Top Gear)',
        { status: 400 },
      );
    }
    ...
    if (!profilesetsData || !profilesetsData.results) {
      return new NextResponse('El reporte no contiene resultados válidos', {
        status: 400,
      });
    }
    ...
    const baselineEntry = results.find(
      (p: any) =>
        p.name === 'Baseline' ||
    ...
    upgrades.forEach((up: any) => {
      ...
      const bonusIds = Array.isArray(up.items?.[0]?.bonus_id)
        ? up.items[0].bonus_id
        : [];
      const translated = RaidbotsParserService.translateBonusIds(bonusIds);
      ...
      if (isNaN(id) && up.items?.length > 0) {
        id = up.items[0].id;
        ilvl = up.items[0].itemLevel || translated.ilvl || 0;
        slot = up.items[0].slot || slot;
      }

      if (ilvl === 0 && translated.ilvl) {
        ilvl = translated.ilvl;
      }
      ...
      if (!itemsById.has(id) || dpsGain > itemsById.get(id).dpsGain) {
        itemsById.set(id, {
          ...
          fullDps: up.mean,
          bonusIds,
          upgradeTrack: translated.track !== 'Normal' ? translated.track : null,
          gems:
            up.items?.[0]?.gems ||
            (up.items?.[0]?.gem ? [up.items[0].gem] : []),
          enchant: up.items?.[0]?.enchant || null,
        });
      }
    ...
    const totalPercentGain =
      baselineDps > 0 ? (totalDpsGain / baselineDps) * 100 : 0;
