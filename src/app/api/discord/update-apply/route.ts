...
export async function POST(req: Request) {
  ...
  try {
    ...
    // Enlaces Externos
    const realmSlug = application.character_realm
      .toLowerCase()
      .replace(/\s+/g, '-');
    ...
    if (rioData?.raid_progression) {
      ...
      let activeRaid = raids.find(
        ([_, r]) =>
          r.mythic_bosses_killed > 0 ||
          r.heroic_bosses_killed > 0 ||
      ...
      embeds: [
        {
          ...
          thumbnail: {
            url:
              ...
              `https://render.worldofwarcraft.com/eu/icons/56/classicon_${charClass.name.toLowerCase().replace(' ', '')}.jpg`,
          },
          author: {
          ...
          fields: [
            ...
            {
              name: 'Spect y Rol',
              value: application.character_spec,
              inline: true,
            },
