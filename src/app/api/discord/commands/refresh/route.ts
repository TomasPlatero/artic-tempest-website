...
export async function POST() {
  try {
    ...
    const commands = dbCommands?.length
      ? dbCommands.map((c) => ({
          name: c.name,
    ...
    if (response.ok) {
      ...
      if (response.status === 403)
        message =
          '403 Forbidden: El bot no tiene permisos o el token es inválido para esta aplicación.';
      if (response.status === 401)
        message = '401 Unauthorized: El Bot Token es incorrecto.';
      if (response.status === 404)
        message = '404 Not Found: El Application ID o Guild ID es incorrecto.';
