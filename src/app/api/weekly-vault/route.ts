...
export async function POST(request: Request) {
	try {
		...
		// Generate unique filename — sanitize user-controlled components
		const safeGuildId = guild_id.replace(/[^a-zA-Z0-9_-]/g, "_").slice(0, 64);
		const safeCharId = character_id
			.replace(/[^a-zA-Z0-9_-]/g, "_")
			.slice(0, 64);
		const safeExt = (file.name.split(".").pop() || "png")
			.replace(/[^a-z0-9]/g, "")
			.slice(0, 10);
		const filename = `${safeGuildId}/${week_start}/${safeCharId}-${Date.now()}.${safeExt}`;
