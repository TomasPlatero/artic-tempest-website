...
export async function GET(req: Request) {
    try {
        ...
        if (!canView) {
            return new NextResponse("No autorizado para ver la lista BiS", { status: 403 })
        }
        ...
        // Filter valid ranks
        const validRanks = Object.keys(visibilityMap).filter(r => visibilityMap[Number(r)] !== false).map(Number)
        ...
        if (membersError || !validMembers) throw membersError || new Error("No members found")
