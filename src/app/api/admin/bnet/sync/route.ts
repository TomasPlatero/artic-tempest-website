...
export async function POST(req: Request) {
    try {
        ...
        const isOfficer = profile?.role_level === "officer" || profile?.role_level === "gm"
        if (!isOfficer) return new NextResponse("Prohibido", { status: 403 })

        const { tierId } = await req.json()
        if (!tierId) return NextResponse.json({ error: "Falta el ID del Tier (ej: 516 para Midnight)." }, { status: 400 })
        ...
        const stream = new ReadableStream({
            async start(controller) {
                ...
                try {
                    ...
                    const expName = typeof expData.name === 'string' ? expData.name : (expData.name?.es_ES || expData.name?.en_US || "Unknown Expansion")
                    ...
                    const relevantInstances = instanceRefs.filter((i: any) => MIDNIGHT_INSTANCES.includes(i.id))

                    let totalItemsSynced = 0;

                    for (const instRef of relevantInstances) {
                        const instRefName = typeof instRef.name === 'string' ? instRef.name : (instRef.name?.es_ES || instRef.name?.en_US)
                        ...
                        for (let x = 0; x < encounters.length; x++) {
                            ...
                            const encRefName = typeof encRef.name === 'string' ? encRef.name : (encRef.name?.es_ES || encRef.name?.en_US)
                            log(`-----> Jefe ${x + 1}/${encounters.length}: ${encRefName} (${encRef.id})`)
                            ...
                            let itemsForBoss = 0;
                            log(`        [⏳] Detectados ${items.length} objetos en la tabla de botín. Analizando...`)
                            ...
                            for (const itemRef of items) {
                                ...
                                try {
                                    ...
                                    if (itemsForBoss % 5 === 0 || itemsForBoss === items.length) {
                                        log(`        [✔] ${itemsForBoss}/${items.length} objetos cacheados localmente...`)
                                    }
                    ...
                    const doneData = JSON.stringify({ done: true, message: `✅ Sincronización completa. Expansión: ${expName}. Instancias: ${relevantInstances.length}. Objetos procesados: ${totalItemsSynced}.` })
