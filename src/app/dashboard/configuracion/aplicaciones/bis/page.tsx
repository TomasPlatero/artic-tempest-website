...
export default function BisSettingsPage() {
  ...
  const handleClearAllBis = async () => {
    if (
      !confirm(
        "¿Estás seguro de que quieres BORRAR todas las listas de bises de TODOS los miembros? Esta acción no se puede deshacer.",
      )
    ) {
      return;
    }
    ...
    try {
      ...
      toast.success("Listas limpiadas", {
        description: "Se han borrado todas las selecciones de BiS.",
      });
      setLogs((prev) => [
        ...prev,
        "=> ELIMINACIÓN MASIVA: Se han borrado todas las listas de bises de la hermandad.",
      ]);
    } catch (e: any) {
  ...
  return (
    <div className="flex flex-col gap-6 py-6 px-4 lg:px-6 w-full animate-in fade-in duration-500">
      <AdminPageHeader
        ...
        backHref="/dashboard/aplicaciones"
      />
        ...
        <div className="md:col-span-5 flex flex-col gap-6">
          <Card className="h-fit">
            <CardHeader>
              ...
              <CardDescription>
                Obtén los datos más recientes de raids, jefes y botín de la
                expansión actual (Midnight).
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="mt-4 bg-muted/50 border rounded-lg p-4 flex flex-col gap-1">
                ...
                <div className="text-xs text-muted-foreground mt-1 ml-6">
                  Este proceso puede tardar varios minutos ya que descarga
                  cientos de objetos de la API oficial y sus correspondientes
                  iconos. Por favor no cierres la ventana.
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="h-fit border-emerald-500/20 bg-emerald-500/5">
            <CardHeader>
              ...
              <CardDescription>
                Genera el código de exportación para el addon
            ...
            <CardContent>
              <Link href="/dashboard/aplicaciones/bis/exportar">
                <Button
                  variant="outline"
                  className="w-full border-emerald-500/20 hover:bg-emerald-500/10 hover:text-emerald-500 transition-colors"
                >
          ...
          {logs.length === 0 ? (
            <div className="flex flex-col items-center justify-center flex-1 border border-dashed rounded-xl bg-card/10 text-center p-6">
              ...
              <p className="text-sm text-muted-foreground max-w-sm mt-2">
                Presiona &quot;Sincronizar Expansión&quot; para iniciar el
                streaming de datos en tiempo real desde los servidores de
                Blizzard.
              </p>
            </div>
          ) : (
