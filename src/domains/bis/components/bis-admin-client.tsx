...
export function BisAdminClient() {
  ...
  const fetchMembers = useCallback(async () => {
    ...
    try {
      ...
      toast.error("Error", {
        description: "No se pudo cargar el resumen de listas.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const fetchMemberSelections = useCallback(
    async (member: MemberSummary) => {
      ...
      try {
        ...
        toast.error("Error", {
          description: "No se pudo cargar la lista del personaje.",
        });
      } finally {
  ...
  const filteredMembers = useMemo(() => {
    return members
      ...
      .sort((a, b) => {
        if (b.selection_count !== a.selection_count)
          ...
          <div className="p-2 space-y-1">
            {loading
              ? Array.from({ length: 8 }).map((_, i) => (
                  <div
                    key={i}
                    className="h-14 w-full bg-muted/20 animate-pulse rounded-lg"
                  />
                ))
                  ...
                  >
                    <div className="relative size-10 flex-shrink-0">
                      <Image
                        ...
                        alt=""
                        fill
                        className="object-cover rounded-md border border-white/5 shadow-lg"
                      />
                    </div>
                    ...
                    <div className="text-right">
                      {m.selection_count > 0 ? (
                        <Badge
                          variant="outline"
                          className="bg-blue-500/10 text-blue-400 border-blue-500/20 text-[10px] font-black"
                        >
                          {m.selection_count}
                        </Badge>
                      ) : (
                        <div className="text-[10px] text-muted-foreground/30 font-black">
                          -
                        </div>
                      )}
                    </div>
        ...
        {!selectedMember ? (
          <div className="flex-1 flex flex-col items-center justify-center text-center text-muted-foreground bg-card/10 border border-dashed border-border/40 rounded-2xl">
            ...
            <h3 className="text-xl font-bold text-foreground">
              Gestión de Listas BiS
            </h3>
            <p className="max-w-md mx-auto mt-2 text-sm">
              Selecciona un personaje de la lista de la izquierda para
              visualizar su lista de deseos completa.
            </p>
          </div>
        ) : (
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                  <div className="relative size-14 shadow-2xl">
                    <Image
                      src={
                        CLASS_IMAGES[selectedMember.class_id] ||
                        "/assets/images/classes/1.jpg"
                      }
                      alt=""
                      fill
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground flex items-center gap-2 mt-0.5">
                      {selectedMember.realm_slug} · {selections.length} ítems
                      seleccionados
                    </p>
                  </div>
                </div>
              ...
              {loadingSelections ? (
                ...
                <div className="h-full flex flex-col items-center justify-center text-center text-muted-foreground p-12">
                  ...
                  <p className="text-sm font-medium">
                    Este personaje aún no tiene ningún ítem seleccionado para
                ...
                <div className="h-full overflow-y-auto">
                  <div className="p-6 grid grid-cols-1 md:grid-cols-2 2xl:grid-cols-3 gap-4">
                    {selections.map((sel) => (
                      <Card
                        key={sel.id}
                        className="group relative overflow-hidden border-border/40 bg-card hover:border-primary/40 transition-all"
                      >
