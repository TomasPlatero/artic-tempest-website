...
const URGENCY_THEMES: Record<string, string> = {
		...
		<div className="space-y-6">
			<Card className="bg-card/40 border-border/40">
				<CardHeader>
					...
					<CardDescription>
						Define la urgencia de reclutamiento para cada especialización. Los
						cambios se reflejan al instante en la web pública.
					</CardDescription>
				</CardHeader>
			</Card>
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 pb-20">
				{classes
					...
					.map((cls: any) => {
						const classConfigs = initialSpots.filter(
							(s: any) => s.class_id === cls.key,
						);
						...
						return (
							<Card
								key={cls.key}
								className="bg-card/40 border-border/40 overflow-hidden shadow-xl hover:shadow-primary/5  group pt-0"
							>
								<CardHeader className="p-6 border-b border-border/40 flex flex-row items-center gap-3 bg-muted/20">
									<div className="relative size-10 rounded shadow-inner overflow-hidden border border-white/10 group-hover:scale-110 transition-transform">
										<Image
											src={`/assets/images/classes/${cls.key}.webp`}
											alt={cls.value}
											fill
											sizes="40px"
									...
									<CardTitle
										className="text-lg font-semibold uppercase tracking-tighter"
										style={{ color: cls.metadata?.color }}
									>
										{cls.value}
									</CardTitle>
								</CardHeader>
								<CardContent className="p-0">
									<div className="divide-y divide-border/20">
										{classSpecs.map((specName: string) => {
											const spot = classConfigs.find(
												(s: any) => s.spec_name === specName,
											);
											...
											return (
												<div
													key={specName}
													className="flex items-center justify-between p-3.5 px-4 group hover:bg-white/5 transition-colors"
												>
													<div className="flex flex-col">
														<span className="text-[14px] font-bold text-white leading-relaxed">
															{specName}
														</span>
														{specName === "Devorador" && (
															<span className="text-[9px] text-blue-400 uppercase font-semibold tracking-tighter">
																🌌 Midnight
															</span>
														)}
													</div>
													<Select
														...
														onValueChange={(val) =>
															void handleUrgencyChange(cls.key, specName, val)
														}
													>
														<SelectTrigger
															className={`w-[130px] h-9 text-[11px] uppercase font-semibold border-none  px-3 ${URGENCY_THEMES[currentUrgency]}`}
														>
															<SelectValue />
														</SelectTrigger>
