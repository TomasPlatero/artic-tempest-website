...
type SettingsGeneralClientProps = {
  ...
  permissions: {
    ...
    discord: PermissionState;
    bnet: PermissionState;
    api: PermissionState;
  };
};
...
export function SettingsGeneralClient({
  ...
  const [savingCreds, setSavingCreds] = useState<
    null | "discord" | "bnet" | "api"
  >(null);

  const canEditGeneral = permissions.general.canEdit;
  const canEditDiscord = permissions.discord.canEdit;
  const canEditBnet = permissions.bnet.canEdit;
  const canEditApi = permissions.api.canEdit;

  const saveCredentials = async (section: "discord" | "bnet" | "api") => {
    const MASK = "••••••••••••••••";
    const payload =
      section === "discord"
        ? {
            discord_client_id: creds.discord_client_id,
            discord_client_secret:
              creds.discord_client_secret !== MASK
                ? creds.discord_client_secret
                : undefined,
            discord_guild_id: creds.discord_guild_id,
          }
        : section === "bnet"
          ? {
              bnet_client_id: creds.bnet_client_id,
              bnet_client_secret:
                creds.bnet_client_secret !== MASK
                  ? creds.bnet_client_secret
                  : undefined,
            }
          : {
              wcl_client_id: creds.wcl_client_id,
              wcl_client_secret:
                creds.wcl_client_secret !== MASK
                  ? creds.wcl_client_secret
                  : undefined,
            };

    setSavingCreds(section);
    try {
      ...
      toast.success("Credenciales guardadas", {
        description:
          "Los cambios se aplicarán en la próxima operación que las utilice.",
      });
      ...
      setCreds((prev) => ({
        ...
        discord_client_secret:
          section === "discord" &&
          prev.discord_client_secret &&
          prev.discord_client_secret !== MASK
            ? MASK
            : prev.discord_client_secret,
        bnet_client_secret:
          section === "bnet" &&
          prev.bnet_client_secret &&
          prev.bnet_client_secret !== MASK
            ? MASK
            : prev.bnet_client_secret,
        wcl_client_secret:
          section === "api" &&
          prev.wcl_client_secret &&
          prev.wcl_client_secret !== MASK
            ? MASK
            : prev.wcl_client_secret,
      }));
    } catch {
  ...
  return (
    <div className="flex flex-col gap-8 p-4 md:p-6 lg:px-8 w-full max-w-full animate-in fade-in duration-500">
              ...
              <div className="grid gap-3 text-sm">
                <div className="flex items-center justify-between">
                  <div className="space-y-0.5">
                    <span className="text-muted-foreground block text-xs uppercase font-bold tracking-tight">
                      Logotipo Principal
                    </span>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex flex-col gap-1 items-end">
                      <input
                        ...
                        disabled={uploading || !canEditGeneral}
                      />
                      <Button
                        variant="outline"
                        size="sm"
                ...
                <div className="flex items-center justify-between">
                  <div className="space-y-0.5">
                    <span className="text-muted-foreground block text-xs uppercase font-bold tracking-tight text-amber-500/80">
                      Logotipo para Móvil
                    </span>
                    <p className="text-[10px] text-muted-foreground/40 leading-tight">
                      Se muestra exclusivamente en la cabecera cuando accedes
                      desde el móvil.
                    </p>
                  </div>
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex flex-col gap-1 items-end">
                      <input
                        ...
                        disabled={uploadingMobile || !canEditGeneral}
                      />
                      <Button
                        variant="outline"
                        size="sm"
      ...
      {(permissions.discord.canView ||
        permissions.bnet.canView ||
        permissions.api.canView) && (
        <>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Configura las credenciales de Discord y Battle.net. Estos
                valores se leen desde la base de datos en lugar del archivo
                .env.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              {permissions.discord.canView && (
                <>
                  ...
                  <div className="space-y-4">
                    {!canEditDiscord && (
                      <p className="rounded-lg border border-amber-500/20 bg-amber-500/5 px-3 py-2 text-xs text-amber-200/80">
                        Puedes consultar esta integracion, pero no modificar sus
                        credenciales.
                      </p>
                    )}
                  ...
                  <div className="pt-2">
                    <Button
                      onClick={() => saveCredentials("discord")}
                      disabled={!canEditDiscord || savingCreds !== null}
                    >
                      {savingCreds === "discord" ? (
                        <>
                        ...
                        <>
                          <IconDeviceFloppy className="size-4 mr-2" /> Guardar
                          Discord
                        </>
                      )}
                    </Button>
              ...
              {permissions.bnet.canView && (
                <>
                      ...
                      <div className="space-y-1.5">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setCreds((prev) => ({
                              ...prev,
                              bnet_client_id: e.target.value,
                            }))
                          }
                          disabled={!canEditBnet}
                        />
                      </div>
                      <div className="space-y-1.5">
                        <label className="text-xs font-medium text-muted-foreground">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setCreds((prev) => ({
                              ...prev,
                              bnet_client_secret: e.target.value,
                            }))
                          }
                          disabled={!canEditBnet}
                        />
                  ...
                  <div className="pt-2">
                    <Button
                      onClick={() => saveCredentials("bnet")}
                      disabled={!canEditBnet || savingCreds !== null}
                    >
                      {savingCreds === "bnet" ? (
                        <>
                          <IconRefresh className="size-4 mr-2 animate-spin" />{" "}
              ...
              {permissions.api.canView && (
                <>
                      ...
                      <div className="space-y-1.5">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setCreds((prev) => ({
                              ...prev,
                              wcl_client_secret: e.target.value,
                            }))
                          }
                          disabled={!canEditApi}
                        />
                  ...
                  <div className="pt-2">
                    <Button
                      onClick={() => saveCredentials("api")}
                      disabled={!canEditApi || savingCreds !== null}
                    >
                      {savingCreds === "api" ? (
                        <>
                          <IconRefresh className="size-4 mr-2 animate-spin" />{" "}
