...
export function SettingsDashboardClient({
  ...
  const toggleActive = async (widget: Widget) => {
    ...
    try {
      ...
      setWidgets((prev) =>
        prev.map((w) =>
          w.id === widget.id ? { ...w, is_active: newStatus } : w,
        ),
      );
  ...
  const saveEdit = async () => {
    try {
      ...
      setWidgets((prev) =>
        prev.map((w) => (w.id === editForm.id ? editForm : w)),
      );
  ...
  const moveWidget = async (id: string, direction: "up" | "down") => {
    ...
    const updatedWithIndexes = newWidgets.map((w, i) => ({
      ...w,
      order_index: i,
    }));
  ...
  const deleteWidget = async (id: string) => {
    if (
      !confirm(
        "¿Estás seguro de que quieres eliminar este widget? Esta acción no se puede deshacer.",
      )
    )
      return;
  ...
  return (
    <div className="flex flex-col gap-8 w-full max-w-full animate-in fade-in duration-500">
      <AdminPageHeader
        title="AJUSTES DE WIDGETS"
        description="Gestiona los widgets que aparecen en la página de inicio del dashboard."
      ...
      <div className="space-y-4">
        <div className="flex flex-col gap-4">
          {widgets.map((widget, index) => (
            <Card
              key={widget.id}
              className={`p-4 ${!widget.is_active ? "opacity-50 grayscale" : ""} border-border/50 bg-card/50 backdrop-blur-sm`}
            >
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-wrap gap-2 mt-1">
                        {["gm", "officer", "raider", "member", "invitado"].map(
                          (role) => {
                            const isActive =
                              editForm.role_levels?.includes(role);
                            return (
                              <Badge
                                key={role}
                    ...
                    <div className="space-y-2">
                      ...
                      >
                        ...
                        <SelectContent>
                          ...
                          <SelectItem value="bis-admin">
                            BiS (Gestión)
                          </SelectItem>
                          <SelectItem value="planificador-cds">
                            Planificador CDs
                          </SelectItem>
                  ...
                  {widget.type === "banner" && (
                    <>
                      <div className="space-y-2">
                        <Label>
                          Título del Banner (soporta {"{guildName}"})
                        </Label>
                        <Input
                  ...
                  {widget.type === "custom" && (
                    <div className="space-y-2 col-span-2">
                      ...
                      <p className="text-[10px] text-muted-foreground italic mt-2">
                        Puees alternar entre el editor visual y el código
                        fuente. El HTML se insertará tal cual.
                      </p>
                    </div>
                  )}
                </div>
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      {widget.app_id && (
                        <Badge
                          variant="outline"
                          className="text-[9px] uppercase border-primary/30 text-primary/80 italic"
                        >
                          App: {widget.app_id}
                        </Badge>
                      )}
            ...
            <div className="grid grid-cols-1 gap-4 py-4">
              ...
              >
                ...
                <div className="text-left">
                  <div className="font-bold text-sm">
                    HTML Personalizado (Template)
                  </div>
                  <div className="text-xs text-muted-foreground whitespace-normal">
                    Crea un widget informativo con diseño premium usando HTML.
                  </div>
                </div>
              </Button>
              <Button
              ...
              >
                ...
                <div className="text-left">
                  ...
                  <div className="text-xs text-muted-foreground whitespace-normal">
                    Imagen de fondo, título y descripción dinámica.
                  </div>
                </div>
              </Button>
