...
export function SettingsNewsClient({
  initialNews,
  categories,
  currentUser,
}: {
    ...
    <div className="flex flex-col gap-8 w-full animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <AdminPageHeader
          ...
          backHref="/dashboard/configuracion/noticias"
        />
        ...
        <div className="flex flex-col lg:flex-row gap-8 items-start">
          ...
          <Card className="flex-1 w-full p-8 border-border/50 bg-card/40 backdrop-blur-md space-y-8 order-2 lg:order-1">
            <div className="space-y-4">
              <Label className="text-lg font-black italic tracking-tighter uppercase opacity-50">
            ...
            <Card className="p-6 border-border/50 bg-card/40 backdrop-blur-md space-y-6 shadow-2xl">
              <div className="flex items-center justify-between border-b border-white/5 pb-4">
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    size="sm"
                    className="rounded-xl h-9 bg-primary text-black font-bold uppercase tracking-widest text-[10px] px-4 shadow-xl shadow-primary/20 hover:scale-105 transition-all"
                    onClick={save}
                  >
                ...
                <div className="pt-4 border-t border-white/5">
                  <div className="flex items-center justify-between p-4 bg-primary/5 border border-primary/10 rounded-2xl">
                    <div className="space-y-0.5">
                      <Label
                        htmlFor="featured"
                        className="cursor-pointer text-[10px] font-black uppercase tracking-widest text-primary"
                      >
                        Noticia Principal
                      </Label>
              ...
              <Table>
                <TableHeader className="bg-white/5">
                  <TableRow className="hover:bg-transparent border-white/5">
                    <TableHead className="text-[10px] font-black uppercase tracking-widest text-zinc-500 py-4 h-auto">
                      Noticia
                    </TableHead>
                    <TableHead className="text-[10px] font-black uppercase tracking-widest text-zinc-500 py-4 h-auto hidden sm:table-cell">
                      Estado
                    </TableHead>
                    <TableHead className="text-[10px] font-black uppercase tracking-widest text-zinc-500 py-4 h-auto hidden md:table-cell">
                      Categoría
                    </TableHead>
                    <TableHead className="text-[10px] font-black uppercase tracking-widest text-zinc-500 py-4 h-auto hidden lg:table-cell">
                      Autor
                    </TableHead>
                    <TableHead className="text-[10px] font-black uppercase tracking-widest text-zinc-500 py-4 h-auto hidden sm:table-cell">
                      Fecha
                    </TableHead>
                    <TableHead className="text-[10px] font-black uppercase tracking-widest text-zinc-500 py-4 h-auto text-right">
                      Acciones
                    </TableHead>
                  </TableRow>
                </TableHeader>
                      ...
                      >
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <div className="font-bold text-sm tracking-tight text-white/90 group-hover:text-primary transition-colors flex items-center gap-2">
                                ...
                                {item.is_featured && (
                                  <Badge className="bg-primary text-black text-[8px] font-black px-1.5 h-4 tracking-tighter flex-shrink-0">
                                    DESTACADA
                                  </Badge>
                                )}
                              </div>
                        ...
                        <TableCell className="hidden sm:table-cell">
                          {item.status === "draft" ? (
                            <Badge
                              variant="outline"
                              className="bg-zinc-500/10 text-zinc-500 border-zinc-500/20 text-[9px] font-black tracking-widest uppercase"
                            >
                              Borrador
                            </Badge>
                          ) : (
                            <Badge
                              variant="outline"
                              className="bg-blue-500/10 text-blue-500 border-blue-500/20 text-[9px] font-black tracking-widest uppercase"
                            >
                              Publicada
                            </Badge>
                          )}
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {news.length === 0 ? (
                <div className="col-span-full p-20 text-center border-2 border-dashed border-white/5 rounded-[40px] bg-white/[0.01]">
                  ...
                  <p className="text-lg font-black italic tracking-tighter uppercase opacity-50">
                    No hay noticias publicadas
                  </p>
                ...
                news.map((item) => (
                  <Card
                    key={item.id}
                    className={`group border-border/40 bg-card/20 hover:bg-card/40 transition-all hover:translate-y-[-4px] overflow-hidden flex flex-col ${item.status === "draft" ? "opacity-70 grayscale-[0.5]" : ""}`}
                  >
                    <div className="relative aspect-video overflow-hidden bg-muted">
                      ...
                      <div className="absolute top-2 right-2">
                        {item.status === "draft" ? (
                          <Badge className="bg-black/60 backdrop-blur-md text-zinc-500 text-[8px] border-zinc-500/30 tracking-widest">
                            BORRADOR
                          </Badge>
                        ) : (
                    ...
                    <div className="p-5 flex flex-col flex-1">
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-2">
                          ...
                          {item.is_featured && (
                            <Badge className="bg-primary text-black text-[9px] font-black uppercase tracking-widest">
                              DESTACADA
                            </Badge>
                          )}
                        </div>
                      ...
                      <h4 className="font-bold text-lg mb-2 line-clamp-2 group-hover:text-primary transition-colors italic leading-tight">
                        {item.title}
                      </h4>
