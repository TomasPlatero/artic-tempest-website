...
const PERMISSION_GROUPS = [
  {
    ...
    apps: [
      ...
      { id: "settings-menu", name: "Navegacion / Menu" },
      { id: "settings-recruitment", name: "Config. Reclutamiento" },
    ],
  },
  {
    id: "integrations",
    label: "Integraciones",
    apps: [
      { id: "settings-discord", name: "Discord" },
      { id: "settings-bnet", name: "Battle.net" },
      { id: "settings-accounts", name: "Cuentas" },
      { id: "settings-api", name: "API / Integraciones" },
      { id: "settings-streamers", name: "Streamers" },
  ...
  },
  {
    ...
    label: "Contenido",
    apps: [
      { id: "settings-news", name: "Noticias" },
      { id: "settings-widgets", name: "Widgets" },
    ],
  },
  {
    id: "apps",
...
export function SettingsRolesClient({
  initialDiscordRoles,
  initialPermissions,
}: SettingsRolesClientProps) {
  const [discordRoles, setDiscordRoles] =
    useState<DiscordRoleConfig[]>(initialDiscordRoles);
  const [permissions, setPermissions] =
    useState<AppPermission[]>(initialPermissions);
  const [creatingRole, setCreatingRole] = useState(false);
  const [editingRoleId, setEditingRoleId] = useState<string | null>(null);
  const [mobileActiveRole, setMobileActiveRole] =
    useState<RoleLevel>("officer");
  const [isMappingModalOpen, setIsMappingModalOpen] = useState(false);

  const [newRoleId, setNewRoleId] = useState("");
  const [newRoleName, setNewRoleName] = useState("");
  const [newRoleLevel, setNewRoleLevel] = useState<RoleLevel>("member");
  const [discordServerRoles, setDiscordServerRoles] = useState<
        ...
        setDiscordRoles((prev) =>
          prev.map((r) =>
            r.role_id === editingRoleId
              ? { role_id: newRoleId, name: newRoleName, level: newRoleLevel }
              : r,
          ),
        );
  ...
  const hasPermission = (
    ...
    if (p) {
      ...
      if (action === "edit") return p.can_manage || p.can_edit;
      if (action === "view") return p.can_manage || p.can_edit || p.can_view;
    }
  ...
  const handlePermissionToggle = async (
    ...
    const nextPermissions = current
      ? permissions.map((p) =>
          p.role_level === level && p.app_id === appId ? updated : p,
        )
      : [...permissions, updated];
        ...
        <TabsContent value="apps" className="pt-8 w-full">
          <Card className="bg-zinc-950/40 border-white/[0.08] backdrop-blur-3xl rounded-[2rem] shadow-2xl ring-1 ring-white/5 overflow-hidden">
            <CardHeader className="p-8 pb-4">
              <CardTitle className="text-xl font-black uppercase tracking-widest text-primary">
                Matriz de Permisos
              </CardTitle>
              <CardDescription className="text-sm font-medium text-white/40">
                Los roles de Miembro e Invitado tienen restricciones por
                defecto.
              </CardDescription>
            </CardHeader>
            <CardContent className="p-4 md:p-8">
              <div className="md:hidden space-y-4">
                <Tabs
                  value={mobileActiveRole}
                  onValueChange={(v) => setMobileActiveRole(v as RoleLevel)}
                  className="w-full"
                >
                  <TabsList className="bg-white/5 p-1 w-full flex justify-start overflow-x-auto no-scrollbar scrollbar-hide h-auto py-2 border border-white/5 rounded-[1.5rem] mb-2">
                    {(
                      ["gm", "officer", "raider", "member", "invitado"] as const
                    ).map((role) => (
                      <TabsTrigger
                        key={role}
                        value={role}
                ...
                <div className="space-y-2.5">
                  {PERMISSION_GROUPS.map((group) => (
                    <div key={group.id} className="space-y-2.5">
                      <div className="px-1 text-[10px] font-black uppercase tracking-[0.25em] text-white/30">
                        {group.label}
                      </div>
                      {group.apps.map((app) => (
                        <div
                          key={app.id}
                          className="bg-white/[0.02] border border-white/[0.05] rounded-2xl p-5 flex flex-col gap-4 group active:bg-white/[0.04] transition-all"
                        >
                ...
                <div className="min-w-full">
                  <div className="grid grid-cols-[1fr_repeat(5,minmax(0,1fr))] bg-white/[0.03] border-b border-white/[0.05] p-5 font-black text-white/30 text-[10px] uppercase tracking-[0.3em]">
                    ...
                    {ROLES.map((role) => (
                      <div
                        key={role.level}
                        className={`col-span-1 text-center ${role.color}`}
                      >
                        {role.label}
                      </div>
                    ))}
                  ...
                  {PERMISSION_GROUPS.map((group) => (
                    <div key={group.id}>
                      <div className="grid grid-cols-[1fr_repeat(5,minmax(0,1fr))] items-center px-5 py-3 border-b border-white/[0.03] bg-white/[0.02]">
                        <div className="col-span-6 text-[10px] font-black uppercase tracking-[0.3em] text-white/35">
                          {group.label}
                        </div>
                      </div>
                      {group.apps.map((app) => (
                        <div
                          key={app.id}
                          className="grid grid-cols-[1fr_repeat(5,minmax(0,1fr))] items-center p-5 border-b border-white/[0.03] last:border-0 hover:bg-white/[0.02] transition-colors group"
                        >
                          <div className="col-span-1 font-black text-white text-base group-hover:text-primary transition-colors">
                            {app.name}
                          </div>
                          {ROLES.map((role) => (
                            <div
                              key={role.level}
                              className="col-span-1 flex justify-center gap-3"
                            >
                              <div className="flex flex-col items-center gap-2 group/switch">
                                ...
                                />
                                <span className="text-[9px] text-zinc-600 group-hover/switch:text-zinc-400 mt-1 uppercase font-black tracking-widest transition-colors">
                                  Ver
                                </span>
                              </div>
                              <div className="flex flex-col items-center gap-2 group/switch">
                    ...
                    .map((role) => (
                      ...
                      >
                        ...
                        <div className="lg:col-span-5 flex flex-col gap-0.5">
                          <span className="font-black text-white text-base group-hover:text-primary transition-colors leading-tight">
                            {role.name}
                          </span>
                          <span className="text-[9px] font-mono font-bold text-zinc-600 uppercase tracking-[0.15em]">
                            {role.role_id}
                          </span>
                        </div>
                        ...
                        <div className="lg:col-span-4 py-1 lg:py-0">
                          <Badge
                            variant="outline"
                            className={`${getRoleDisplay(role.level).badge} px-3 py-1 rounded-xl text-[9px] font-black uppercase tracking-[0.15em] border-2 shadow-sm`}
                          >
                        ...
                        <div className="lg:col-span-3 flex justify-end gap-2.5 w-full lg:w-auto lg:translate-x-4 lg:opacity-0 lg:group-hover:opacity-100 lg:group-hover:translate-x-0 transition-all duration-300">
                          <Button
                            variant="outline"
                            size="icon"
                            className="size-10 rounded-xl border-white/10 bg-white/5 hover:bg-white/10 hover:border-white/20 shadow-xl transition-all"
                          ...
                          <Button
                            variant="outline"
                            size="icon"
                            className="size-10 rounded-xl border-rose-500/10 bg-rose-500/5 text-zinc-500 hover:text-rose-500 hover:bg-rose-500/10 hover:border-rose-500/20 shadow-xl transition-all"
                            onClick={() =>
                              handleDeleteDiscordMapping(role.role_id)
                            }
                          >
        ...
        <DialogContent className="max-w-md bg-zinc-950 border-white/10 text-white rounded-3xl shadow-[0_0_50px_rgba(0,0,0,0.8)] p-0 overflow-hidden">
          ...
          <div className="p-8 space-y-6">
            <div className="space-y-2">
              <Label className="text-[10px] uppercase font-black text-white/30 ml-1 tracking-[0.2em]">
                ...
                <SelectContent className="bg-zinc-900 border-white/10 text-white rounded-xl max-h-[300px]">
                  {discordServerRoles.map((role) => (
                    <SelectItem
                      key={role.id}
                      value={role.id}
                      className="text-[11px] font-bold py-3"
                    >
                      {role.name}
                    </SelectItem>
                  ))}
            ...
            <div className="space-y-2">
              ...
              <Select
                value={newRoleLevel}
                onValueChange={(v: RoleLevel) => setNewRoleLevel(v)}
              >
